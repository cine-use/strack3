...
function Editor( container_id, options )
{
	...
	graphcanvas.background_image = StrackPHP['IMG']+"/node/grid.png";
	graph.onAfterExecute = function() { graphcanvas.draw(true) };

	//add stuff
	//this.addToolsButton("loadsession_button","Load",StrackPHP['IMG']+"/node/icon-load.png", this.onLoadButton.bind(this), ".tools-left" );
	//this.addToolsButton("savesession_button","Save",StrackPHP['IMG']+"/node/icon-save.png", this.onSaveButton.bind(this), ".tools-left" );
	this.addLoadCounter();

	// this.addToolsButton("playnode_button","Play",StrackPHP['IMG']+"/node/icon-play.png", this.onPlayButton.bind(this), ".tools-right" );
	// this.addToolsButton("playstepnode_button","Step",StrackPHP['IMG']+"/node/icon-playstep.png", this.onPlayStepButton.bind(this), ".tools-right" );
	//
	// this.addToolsButton("livemode_button","Live",StrackPHP['IMG']+"/node/icon-record.png", this.onLiveButton.bind(this), ".tools-right" );
	// this.addToolsButton("maximize_button","",StrackPHP['IMG']+"/node/icon-maximize.png", this.onFullscreenButton.bind(this), ".tools-right" );
...
Editor.prototype.onPlayButton = function()
{
	...
	{
		button.innerHTML = "<img src='"+StrackPHP['IMG']+"/node/icon-stop.png'/> Stop";
		graph.start(1); 
	}
	else
	{
		button.innerHTML = "<img src='"+StrackPHP['IMG']+"/node/icon-play.png'/> Play";
		graph.stop(); 
	}
};
...
Editor.prototype.onLiveButton = function()
{
	...
	var url = this.graphcanvas.live_mode ? StrackPHP['IMG']+"/node/gauss_bg_medium.jpg" : StrackPHP['IMG']+"/node/gauss_bg.jpg";
	var button = this.root.querySelector("#livemode_button");
	button.innerHTML = !is_live_mode ? "<img src='"+StrackPHP['IMG']+"/node/icon-record.png'/> Live" : "<img src='"+StrackPHP['IMG']+"/node/icon-gear.png'/> Edit" ;
};
