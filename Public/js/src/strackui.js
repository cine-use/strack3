...
(function ($) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (opts.showContextMenu) {
                        ...
                        right_menu_dom(opts.contextMenuData);
                    }
    ...
    function build_filter(dg, opts, bardom, maindom, data) {
        ...
        if (opts.authorityRules.filter.show === "yes") {
            ...
            //过滤条件tag列表
            $fid.empty()
                .append(term_wrap_dom(dom, opts.searchConfig.bar_show))
                .append(filter_main_wrap(opts.authorityRules.filter.edit))
        ...
        if (opts.toolbarConfig.groupAllow && opts.authorityRules.group === "yes") {
            $('#' + opts.toolbarConfig.id).find(".grid_group").append(Strack.toolbar_down_init('group', data["group_list"], opts, 'grid'));
        }
    ...
    //过滤面板包裹
    function filter_main_wrap(auth_edit) {
        var dom = '';
        dom += ' <div class="grid-filter-right filter-adv-list">' +
            filter_main_header(auth_edit) +
    ...
    //过滤面板头部
    function filter_main_header(auth_edit) {
        var dom = '';
        ...
        dom +='<a href="javascript:;" class="item" onclick="Strack.reset_filter_default(this);">' + StrackLang["Reset_Default"] + '</a>' +
            '<a href="javascript:;" class="item" onclick="Strack.open_advance_filter(this);">' + StrackLang["Advanced_Filter"] + '</a>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>';
    ...
    //过滤条件包裹 DOM
    function term_wrap_dom(fdom, bar_show) {
        unsave_filter_item();
    ...
    //生成右键菜单 DOM
    function right_menu_dom(param) {
        if ($('#' + param["id"]).length === 0) {
            var dom = '';
            param["data"].forEach(function (val) {
                if (val["children"]) {
                    dom += menu_item_dom("multi", val);
                } else {
                    dom += menu_item_dom("single", val);
                }
            });
    ...
    function menu_item_dom(type, data) {
        var dom = "",
            options = "iconCls:'" + data["iconCls"] + "'";
        var attr_lang = '';
        switch (type) {
            case "single":
                dom += '<div id="st_menu_' + data["id"] + '" data-options="' + options + '" ' + menu_item_atrr(data) + '>' + data["lang"] + '</div>';
                break;
            case "multi":
                var tempOptions = '';
                dom += '<div id="st_menu_' + data["id"] + '" data-options="' + options + '" ' + menu_item_atrr(data) + '>' + data["lang"] + '<div>';
                data["children"].forEach(function (val) {
                    ...
                    dom += '<div id="st_menu_' + val["id"] + '" data-options="' + tempOptions + '" ' + menu_item_atrr(val) + '>' + val["lang"] + '</div>';
                });
                dom += '</div></div>';
                break;
        }
    ...
    var _790 = {
        ...
        onAfterRender: function (_7d3) {
            ...
            //当datagrid没有值得时候
            var dnum = $(_7d3).datagrid("getData");
            if (parseInt(dnum["total"]) === 0 && opts.dataGridType === "datagrid") {
