...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    rows_group_h: {}, // 每层包体高度
    setEmptyMsg: '',
    load_status: false, // 加载状态控制
    rows: [],
    render: function(target, container, frozen, mode){
        ...
        if(mode === "new"){
            ...
            for(var i=0; i<groups.length; i++){
                table.push( this.renderGroup.call(this, target, i, groups[i], frozen, mode));
            }
            ...
            var more_rows = '';
            for(var i=0; i<groups.length; i++){
                more_rows += this.renderGroup.call(this, target, i, groups[i], frozen, mode);
            }
            ...
            if(opts.groupActive){
                ...
                $(container).find(".datagrid-btable-bottom").before(more_rows);
            }else {
                $(container).find("table").append(more_rows);
            }
        }
    ...
    // 渲染分组
    renderGroup: function(target, groupIndex, group, frozen, mode){
        var state = $.data(target, 'datagrid');
        var opts = state.options;
        ...
        if(opts.groupActive){
            ...
            if ((frozen && (opts.rownumbers || opts.frozenColumns.length)) ||
                ...
                rows_dom.push('<td style="border:0;text-align:center;width:25px"><span class="datagrid-row-expander datagrid-row-collapse icon-uniF077" style="display:inline-block;margin-left: 8px;cursor:pointer">&nbsp;</span></td>');
            }
            rows_dom.push('<td style="border:0;">');
            if (!frozen) {
                //未冻结
                rows_dom.push('<span class="datagrid-group-title unfrozen_gtitle text-ellipsis">');
                if (opts.frozenColumns[0]) {
                    if (opts.frozenColumns[0].length < 2) {
                        rows_dom.push(opts.groupFormatter.call(target, group.value, group.rows));
                    }
                }
                rows_dom.push('</span>');
            } else {
                //冻结
                rows_dom.push('<span class="datagrid-group-title frozen_gtitle text-ellipsis">');
                if (opts.frozenColumns[0]) {
                    if (opts.frozenColumns[0].length >= 2) {
                        rows_dom.push(opts.groupFormatter.call(target, group.value, group.rows));
                    }
            ...
            table.push('<div class="datagrid-group" class="grid-r-g-'+this.page+'" group-index=' + groupIndex + '>');
            table.push('<table cellspacing="0" cellpadding="0" border="0" style="height:100%"><tbody>');
            table.push(rows_dom.join(""));
        ...
        for(var j=0; j<group.rows.length; j++) {
            ...
            tr_rows_dom.push('<tr id="' + rowId + '" datagrid-row-index="' + index + '" ' + cls + ' ' + style + '>');
            tr_rows_dom.push(this.renderRow.call(this, target, fields, frozen, index, group.rows[j]));
            tr_rows_dom.push('</tr>');
            index++;
        }

        table.push('<table class="datagrid-btable grid-r-g-'+this.page+'" cellspacing="0" cellpadding="0" border="0"><tbody>');
        table.push(tr_rows_dom.join(""));
        table.push('</tbody></table>');

        if(mode === "new" || opts.groupActive){
            return table.join('');
        }else {
            return '<tbody class="grid-r-g-'+this.page+'">'+tr_rows_dom.join('')+'</tbody>';
        }
    },
    onBeforeRender: function(target, rows){
        ...
        this.r1 = this.r3 = [];	// the first part and last part of rows
    ...
    scrolling: function(target){
        ...
        if (opts.finder.getRows(target).length === 0){
            ...
            this.getNextRows.call(this, target, 1);
        } else {
            ...
            // 优化滚动加载
            // console.log(topDiv._outerHeight());
            // console.log(this.page);
            // console.log(top);
            // console.log(bottom);

            if(this.page>1 && Math.abs(top) <= parseInt(bottom)){
                // 向上滚动
                console.log("向上滚动");
            }else {
                // 向下滚动
                ...
                if (bottom - 350 < dc.body2.height()){
                    ...
                    this.populate.call(this, target, 'more');
                    console.log("page:"+page);
                    this.getNextRows.call(this, target, page);
    ...
    getNextRows: function(target, page){
        this.getRows.call(this, target, page+1, function(rows){
            this.r3 = rows;
        });
    },
    reload: function(target){
        ...
        this.getRows.call(this, target, page, function(rows){
            ...
            this.r2 = rows;
            this.r3 = [];
            this.populate.call(this, target, 'new');
            dc.body2.triggerHandler('scroll.datagrid');
        });
    },
    ...
    populate: function(target, mode){
        ...
        if (this.rows.length){
            opts.view.render.call(opts.view, target, dc.body2, false, mode);
            opts.view.render.call(opts.view, target, dc.body1, true, mode);
            ...
            this.rows_group_h["r_"+page] = resetRowHeight(opts, dc, page);

            console.log( this.rows_group_h);
        ...
        // 重置行高
        function resetRowHeight(opts, dc, page) {
            var row_index = 0;
            var page_g_h = 0;
            if(opts.differhigh){
                var r_h = 0;
                dc.body2.find(".grid-r-g-"+page+" .datagrid-row").each(function (index) {
                    row_index = $(this).attr("datagrid-row-index");
                    // 记录当前分组集合高度
                    r_h = $(this).height();
                    page_g_h += r_h+1;
                    dc.body1.find("#datagrid-row-r1-1-"+row_index).css("height", r_h);
                });
            }else {
    ...
    resizeGroup: function(target, groupIndex){
        ...
        if (groupIndex === undefined){
            groupHeader = dc.body2.children('div.datagrid-group');
        } else {
            groupHeader = dc.body2.children('div.datagrid-group[group-index=' + groupIndex + ']');
        }
