...
class FillAppendUpdateDefaultViewRules extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function up()
    {
        ...
        $pageAuthLinkData = $this->fetchAll('SELECT id  FROM strack_page_auth WHERE code="save_default_view" and param in (54,55,56,57,58,59,60)');

        foreach ($pageAuthLinkData as $item) {
            $this->execute('UPDATE `strack_page_auth` SET `page`="home_project_entity" WHERE (`id`= "' . $item["id"] . '")');
        }
    }
