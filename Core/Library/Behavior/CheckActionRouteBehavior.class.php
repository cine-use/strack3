...
class CheckActionRouteBehavior
{
    ...
    private function parseRule($rule, $route, $regx)
    {
        ...
        if (0 === strpos($url, '/') || 0 === strpos($url, 'http')) {
            ...
            if (strpos($url, ':')) { // 传递动态参数
                ...
                $url = preg_replace_callback('/:(\d+)/', function ($findVal) use (&$values) {
                    return $values[$findVal[1] - 1];
                }, $url);
            }
            ...
            if ($paths) {
                ...
                preg_replace_callback('@(\w+)\/([^\/]+)@', function ($findVal) use (&$var) {
                    $var[strtolower($findVal[1])] = strip_tags($findVal[2]);
                }, implode('/', $paths));
            }
            // 解析路由自动传入参数
    ...
    private function parseRegex($matches, $route, $regx)
    {
        ...
        $url = preg_replace_callback('/:(\d+)/', function ($findVal) use ($matches) {
            return $matches[$findVal[1]];
        }, $url);
        if (0 === strpos($url, '/') || 0 === strpos($url, 'http')) {
            // 路由重定向跳转
            header("Location: $url", true, (is_array($route) && isset($route[1])) ? $route[1] : 301);
